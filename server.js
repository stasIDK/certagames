...
function processBotTurns(room){if(!room?.game||room.game.phase==='done')return;const g=room.game;const currentIdx=g.phase==='defend'?g.defenderIdx:g.attackerIdx;const currentBot=room.players[currentIdx];if(!currentBot?.isBot)return;const hand=g.hands[currentBot.sub];const role=g.phase==='defend'?'defend':(g.table.length===0?'attack':'attack-more');if(role==='attack'||role==='attack-more'){const defender=room.players[g.defenderIdx];const defHandSize=g.hands[defender.sub]?.length??0;if(g.table.length>=defHandSize){if(g.table.length>0){g.table=[];const n=room.players.length;g.attackerIdx=g.defenderIdx;g.defenderIdx=(g.defenderIdx+1)%n;g.phase='attack';refillHands(room);advancePastFinished(room);if(checkGameOver(room)){endGame(room);return;}broadcastGameState(room);setTimeout(()=>processBotTurns(room),4500);}return;}const moveRole=role==='attack'?'attack':'pile-on';const card=botMove(currentBot.difficulty,hand,g.table,g.trump,moveRole);if(!card){if(g.table.length>0){g.table=[];const n=room.players.length;g.attackerIdx=g.defenderIdx;g.defenderIdx=(g.defenderIdx+1)%n;g.phase='attack';refillHands(room);advancePastFinished(room);if(checkGameOver(room)){endGame(room);return;}broadcastGameState(room);setTimeout(()=>processBotTurns(room),4500);}return;}const idx=hand.findIndex(c=>c.r===card.r&&c.s===card.s);if(idx===-1)return;hand.splice(idx,1);g.table.push({attack:card,defense:null});g.phase='defend';broadcastGameState(room);setTimeout(()=>processBotTurns(room),5000);return;}if(role==='defend'){const card=botMove(currentBot.difficulty,hand,g.table,g.trump,'defend');if(!card){const allCards=g.table.flatMap(p=>[p.attack,p.defense].filter(Boolean));g.hands[currentBot.sub].push(...allCards);g.table=[];const n=room.players.length;const na=(g.defenderIdx+1)%n;g.attackerIdx=na;g.defenderIdx=(na+1)%n;g.phase='attack';refillHands(room);advancePastFinished(room);if(checkGameOver(room)){endGame(room);return;}broadcastGameState(room);setTimeout(()=>processBotTurns(room),4500);return;}const idx=hand.findIndex(c=>c.r===card.r&&c.s===card.s);if(idx===-1)return;hand.splice(idx,1);const pair=g.table.find(p=>!p.defense);if(pair)pair.defense=card;const allDefended=g.table.every(p=>p.defense);if(allDefended){g.phase='attack';broadcastGameState(room);setTimeout(()=>processBotTurns(room),4500);}else{broadcastGameState(room);setTimeout(()=>processBotTurns(room),5000);}return;}}
...
function collectPokerBets(room){const g=room.game;for(const sub of Object.keys(g.bets)){g.pot+=(g.bets[sub]||0);g.bets[sub]=0;}g.currentBet=0;g.lastRaiser=null;}
